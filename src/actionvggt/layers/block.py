...
class Block(nn.Module):
    ...
    def forward(
        ...
        def attn_residual_func(x: Tensor, pos=None, attn_mask=None, past_key_values=None, use_cache=False) -> Union[Tensor, Tuple[Tensor, Dict]]:
            if use_cache:
                norm_x = self.norm1(x)
                output, new_kv = self.attn(norm_x, pos=pos, past_key_values=past_key_values, use_cache=True)
                return self.ls1(output), new_kv
            else:
                norm_x = self.norm1(x)
                if attn_mask is not None:
                    out = self.attn(norm_x, pos=pos, attn_mask=attn_mask)
                else:
                    out = self.attn(norm_x, pos=pos)
                return self.ls1(out)
...
class CrossBlock(nn.Module):
    ...
    def forward(
        ...
        def attn_residual_func(x: Tensor, pos=None, attn_mask=None, past_key_values=None, use_cache=False):
            norm_x = self.norm1(x)
            if use_cache:
                ...
                return self.ls1(out), new_kv
            out = self.attn(norm_x, context, pos=pos, attn_mask=attn_mask)
            return self.ls1(out)
